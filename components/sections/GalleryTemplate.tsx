...
export function GalleryTemplate({
  ...
  const backUrl = mainCategory
    ? `/photography/${mainCategory}`
    : `/photography`;
  const backText = mainCategory
      ...
      <header className="sticky top-20 z-40 border-b border-border bg-background/80 backdrop-blur-md">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <Link
            ...
            className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors relative z-50"
          >
        ...
        <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-6 space-y-6">
          {allImages.length > 0 ? (
            ...
            <div className="col-span-full text-center py-24 text-muted-foreground bg-white/5 rounded-2xl border border-white/10 border-dashed">
              No images have been uploaded to this category yet. Check back
              soon!
            </div>
          )}
        </div>
