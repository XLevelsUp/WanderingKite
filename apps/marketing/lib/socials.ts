/**
 * Social profile data shared by the footer and any other surface that links
 * to the brand's accounts (e.g. the blog post sidebar).
 *
 * Deliberately NOT inside Footer.tsx: that file is a client component, and a
 * plain data export cannot cross the server/client boundary — a server
 * component importing it receives a module proxy, not the array, which fails
 * at runtime with "brandSocials.map is not a function". This module carries no
 * 'use client' directive, so both sides can import it directly.
 */
...
// Exported so other surfaces — e.g. the blog post sidebar — link to the same
// profiles rather than keeping a second copy of these URLs.
