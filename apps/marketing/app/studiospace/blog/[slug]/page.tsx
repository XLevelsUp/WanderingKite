...
export async function generateMetadata({
  ...
  const post = await getBlogPostBySlug(slug, 'STUDIO');
  ...
  return {
    title,
    description,
    alternates: { canonical: `${BASE_PATH}/${post.slug}` },
    openGraph: {
      title,
      description,
      type: 'article',
...
export default async function StudioBlogPostPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const structuredData: object[] = [
    {
      ...
      '@type': 'BlogPosting',
      '@id': `${SITE_URL}${BASE_PATH}/${post.slug}#article`,
      headline: post.title,
      description,
      ...
      publisher: {
        '@type': 'Organization',
        name: 'Studio Space Coimbatore',
        logo: { '@type': 'ImageObject', url: `${SITE_URL}/logo.svg` },
      },
      mainEntityOfPage: {
        '@type': 'WebPage',
        '@id': `${SITE_URL}${BASE_PATH}/${post.slug}`,
      },
      ...
      <main className="min-h-screen pt-32">
        <article>
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="mb-8">
              <Link
                href={`${BASE_PATH}?category=${CATEGORY_SLUGS[post.category as BlogCategory]}`}
                    ...
                    <div className="border-t border-border pt-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {post.tags.map((tag: string) => (
                          <Link
                            key={tag}
                            href={`${BASE_PATH}?tag=${encodeURIComponent(tag)}`}
