...
export function PortfolioCard({ shoot, coverImage }: PortfolioCardProps) {
  return (
    <Link href={`/photography/${shoot.category?.toLowerCase() || 'general'}`} className="block group">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col justify-end">
          <div className="flex items-end justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold tracking-tight text-white">{shoot.title}</h3>
              {shoot.description && (
                <p className="mt-2 text-sm text-zinc-400 line-clamp-2">{shoot.description}</p>
              )}
            </div>
