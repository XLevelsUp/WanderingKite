...
export async function resolveStudioBookingConflict(
  ...
  try {
    ...
    revalidatePath('/booking-conflicts');
    revalidatePath('/');
    // Note: /client/dashboard is a customer-portal route in apps/marketing
    // (a separate deployment) and can't be revalidated in-process from here.
    return { success: true };
  } catch (error: any) {
