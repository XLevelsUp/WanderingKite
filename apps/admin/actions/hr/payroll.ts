...
export async function getPayslip(recordId: string): Promise<PayrollRecordWithEmployee | null> {
  ...
  if (!isAdmin && row.employeeId !== user.id) redirect('/');
...
export async function updatePayrollOverride(
  ...
  revalidatePath('/hr/payroll');
  revalidatePath(`/hr/payroll/${record.year}-${String(record.month).padStart(2, '0')}`);
  return { success: true };
}
...
export async function deleteBatchPayroll(month: number, year: number) {
  ...
  revalidatePath('/hr/payroll');
  revalidatePath(`/hr/payroll/${year}-${String(month).padStart(2, '0')}`);
  return { success: true };
}
