...
import { BLOG_EDITOR_ROLES, estimateReadingTime, type BlogCommentStatus } from '@/lib/blog';
...
/**
 * Revalidate the admin-side blog CMS listing.
 *
 * The public blog surfaces (/blog and /studiospace/blog in apps/marketing)
 * live in a separate deployment now and cannot be revalidated in-process
 * from here — revalidatePath only affects the Next.js cache of the app
 * that calls it. Marketing needs its own on-demand-revalidation trigger
 * (e.g. a webhook hit on publish/edit) if it should pick up changes before
 * its own ISR window naturally expires; that's tracked as an open item,
 * not solved here.
 */
...
export async function updateBlogPost(id: string, input: unknown) {
  ...
  try {
    ...
    revalidateBlog(post.slug);
    // Note: the old/new slug's public detail pages (apps/marketing) are a
    // separate deployment and can't be revalidated in-process from here —
    // see revalidateBlog's comment above.
    return { success: true, id: post.id, slug: post.slug };
  } catch (err: any) {
...
export async function setBlogCommentStatus(id: string, status: BlogCommentStatus) {
  try {
    ...
    // Note: the public post page (apps/marketing) can't be revalidated
    // in-process from here — see revalidateBlog's comment above.
    return { success: true };
  } catch (err: any) {
