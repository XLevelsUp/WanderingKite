...
export function StudioPricingEngine({ equipment = [] }: { equipment?: any[] }) {
    ...
    const subtotal = useMemo(() => {
        ...
        return total;
    }, [selectedPackage, selectedAddOns, selectedEquipment, packageHours, equipment]);
            ...
            <div className="grid lg:grid-cols-12 gap-8 lg:gap-12">
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <addon.icon className={`w-4 h-4 ${isSelected ? 'text-warning' : 'text-zinc-500'}`} />
                ...
                {equipment.length > 0 && (
                    <div className="lg:col-span-12 mt-4 mb-2">
                        <div className="mb-6">
                            <h3 className="text-2xl font-bold text-white">In-Studio Equipment Add-ons</h3>
                            <p className="text-zinc-400 mt-1">Rent gear directly for your studio session</p>
                        </div>
                        ...
                        <div className="space-y-8">
                            {[
                                ...
                                const items = equipment.filter(e => (e.categories as any)?.name?.toLowerCase().includes(cat.key));
                                if (items.length === 0) return null;
                                return (
                                    <div key={cat.key}>
                                        <h4 className="text-lg font-semibold text-white mb-4">{cat.title}</h4>
                                                    ...
                                                    >
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="flex items-center justify-between mt-2">
                                                                <span className="text-warning font-mono text-sm">{formatINR(hourly)}/hr</span>
                                                                <button 
                        ...
                        {selectedEquipment.size > 0 && (
                            <div className="mt-4 flex justify-end">
                                <div className="rounded-xl border border-warning/30 bg-warning/5 px-6 py-3 text-sm flex items-center gap-2">
                                    <span className="text-zinc-400">Studio Equipment Add-ons: </span>
                                    <span className="text-warning font-mono font-bold text-lg">
                                        {formatINR(Array.from(selectedEquipment).reduce((acc, id) => {
                                            const eq = equipment.find(e => e.id === id);
                                            return acc + (eq ? Math.round((eq.rentalPrice || 0) / 8) * packageHours : 0);
                                        }, 0))}
                                    </span>
                                </div>
                            </div>
