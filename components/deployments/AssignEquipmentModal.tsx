...
interface AssignEquipmentFormContentProps extends AssignEquipmentModalProps {
  onClose: () => void;
  isPending: boolean;
  startTransition: React.TransitionStartFunction;
}

function AssignEquipmentFormContent({
  employees,
  equipment,
  clients,
  isEmployee,
  ...
  isPending,
  startTransition,
}: AssignEquipmentFormContentProps) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/8 shrink-0">
          ...
          <button
            ...
            onClick={onClose}
            disabled={isPending}
            className="p-1.5 rounded-lg text-foreground/40 hover:text-foreground/70 hover:bg-white/5 transition-colors disabled:opacity-40"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
                      ...
                      <label key={eq.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${isConflict ? 'opacity-50 grayscale bg-red-500/5 hover:bg-red-500/10' : 'hover:bg-white/5'}`}>
                        ...
                        {isConflict && (
                          <span className="text-[10px] font-medium text-red-400 bg-red-500/10 px-2 py-0.5 rounded-md flex items-center gap-1">
                            <AlertTriangle className="w-3.5 h-3.5" /> Booked
                          </span>
                        )}
                      </label>
                    );
